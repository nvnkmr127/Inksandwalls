...
export interface ValidatedCollectionData {
  name: string;
  slug: string;
  description: string | null;
  isActive: boolean;
...
/**
 * Validates and normalizes collection mutation payload server-side.
 */
export function validateCollectionInput(input: unknown): ValidatedCollectionData {
  ...
  if (name.length > 100) {
    throw new ValidationError("Collection name cannot exceed 100 characters.");
  }
  ...
  if (!slug || slug.length === 0) {
    throw new ValidationError("A valid URL-safe collection slug could not be generated.");
  }

  if (slug.length > 100) {
    throw new ValidationError("Collection slug cannot exceed 100 characters.");
  }
  ...
  if (data.description !== undefined && data.description !== null) {
    if (typeof data.description !== "string") {
      throw new ValidationError("Collection description must be a string.");
    }
    const trimmedDesc = data.description.trim();
    if (trimmedDesc.length > 1000) {
      throw new ValidationError("Collection description cannot exceed 1000 characters.");
    }
